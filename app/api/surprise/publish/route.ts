...
export async function POST(request: Request) {
  try {
    ...
    const slug = makeSlug(surprise.title);

    /*
     * IMPORTANT:
     * Password code is NOT sent to the public frontend.
     *
     * For the first production version we store it in the
     * password_hash column. Later we can replace this with
     * a proper bcrypt/argon2 hash without changing the UI.
     */

    const passwordEnabled = Boolean(surprise.password?.enabled);
    ...
    const { data: universe, error: universeError } = await supabase
      ...
      .insert({
        owner_id: user.id,
        slug,
        title: surprise.title || "A Little Surprise For You ♡",
        ...
        password_hash: passwordEnabled
          ? surprise.password.code
          : null,
        password_screen: {
          hint: surprise.password?.hint || "",
        },
        published: true,
      })
      .select("id, slug")
    ...
    if (universeError || !universe) {
      console.error("Universe insert error:", universeError);
    ...
    const pages = [
       ...
       */
      {
        ...
        page_order: 2,
        page_type: "memories",
        title: surprise.memories?.title || "Our little memories ♡",
        content: surprise.memories?.intro || "",
        settings: {
          eyebrow: surprise.memories?.eyebrow || "CHAPTER 01",
          bottomText:
       ...
       */
      {
        ...
        content: surprise.birthday?.message || "",
        settings: {
          eyebrow:
            surprise.birthday?.eyebrow ||
            "✦ A LITTLE CELEBRATION ✦",
          forYouText:
            surprise.birthday?.forYouText || "FOR YOU",
        },
      },
       ...
       */
      {
        ...
        content: surprise.reasons?.subtitle || "",
        settings: {
          eyebrow:
            surprise.reasons?.eyebrow || "CHAPTER 03",
          items: surprise.reasons?.items || [],
        },
      },
       ...
       */
      {
        ...
        content: surprise.letter?.content || "",
        settings: {
          eyebrow:
            surprise.letter?.eyebrow || "CHAPTER 04",
          signature:
            surprise.letter?.signature || "With love ♡",
        },
      },
     ...
     * MEMORY MEDIA
     */
    const memoryItems = surprise.memories?.items || [];
    ...
    console.error("Publish surprise error:", error);

    return NextResponse.json(
      {
        error: "Something went wrong while publishing the surprise.",
      },
      { status: 500 }
    );
  }
